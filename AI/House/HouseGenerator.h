...
public:
	...
	static void GenerateVerticalWall(int id, AStar& navmesh, AIPlayroom& room,int y1,int y2, int x);

	static void GenerateHorizontalWall(int id,AStar& navmesh, AIPlayroom& room, int x1, int x2, int y);
	static void MarkAreaAsHouse(AStar& navmesh, int x1, int x2, int y1,int y2);

	static House* FindHouseByID(int id);
};
