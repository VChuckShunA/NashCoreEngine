...
AshuraScore::AshuraScore(GameEngine* gameEngine,int Score) : Scene(gameEngine), SCORE(Score)
{
    init();
}
void AshuraScore::init()
{
    registerAction(sf::Keyboard::Space, "PLAY");
    registerAction(sf::Keyboard::Escape, "QUIT");

    m_title = "YOUR SCORE : " + std::to_string(SCORE);
    int titleSize = 60;
    ...
    m_menuStrings.emplace_back("Press Space to Play Again");
...
void AshuraScore::update()
{
    sRender();
}

void AshuraScore::onEnd()
{
    m_game->quit();
}

void AshuraScore::sDoAction(const Action& action)
{
